...
//	static final boolean COMPETITION = false;
	private static final boolean PROBLEMS_1_TO_8_ONLY = true;
//	private static final boolean PROBLEMS_1_TO_8_ONLY = false;
	...
	private void launch ()
	{
		...
		if (Main.PROBLEMS_1_TO_8_ONLY)
		{
			ArrayList <Problem> filtered = new ArrayList <Problem> ();
			for (Problem problem : problems)
			{
				Integer index = Main.extractProblemIndex (problem.getName ());
		...
		Reflections reflections = new Reflections ("bezier.projects");
	...
	private static Integer extractProblemIndex (String problemName)
	{
		int i = problemName.length () - 1;
		while (i >= 0 && Character.isDigit (problemName.charAt (i)))
			i--;
		if (i == problemName.length () - 1)
			return null;
		try
		{
			return Integer.parseInt (problemName.substring (i + 1));
		}

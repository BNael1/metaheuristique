...
/**
 * Grid-BIPOP-CMA-ES : Quality-Diversity guided restarts.
 *
 * Discrétise l'espace 2D en une grille NxN. Pour chaque individu évalué,
 * on regarde la case traversée par le milieu de la courbe (t=0.5 approximé
 * par le point de contrôle médian). Une archive MAP-Elites stocke le meilleur
 * individu par case. Au restart, on choisit préférentiellement une case peu
 * explorée pour forcer CMA-ES à optimiser des routes différentes.
 *
 * Ceci élimine la convergence répétée vers la même impasse sur prob4.
 */
public class CMAESGridBipop implements Optimizer
{
    // ===== Références externes =====
    ...
    // ===== Quality-Diversity Archive (MAP-Elites grid) =====
    private static final int GRID_SIZE = 20;
    private double [][] archiveSolutions;   // [GRID_SIZE * GRID_SIZE][d]
    private double [] archiveFitness;       // [GRID_SIZE * GRID_SIZE]
    private int [] archiveVisits;           // [GRID_SIZE * GRID_SIZE]
    private double gridMinX, gridMinY, gridCellW, gridCellH;

    // ===== Constructeur =====
    public CMAESGridBipop (Problem problem, int d, double [] lb, double [] ub, double [] initMean)
    {
    ...
    public void init ()
    {
        ...
        Arrays.fill (archiveFitness, Double.POSITIVE_INFINITY);

        gridMinX = bounds.getLb () [0];
        gridMinY = bounds.getLb () [1];
        gridCellW = (bounds.getUb () [0] - bounds.getLb () [0]) / GRID_SIZE;
        gridCellH = (bounds.getUb () [1] - bounds.getLb () [1]) / GRID_SIZE;
    ...
    /**
     * Calcule la case de la grille pour un individu, en prenant le point de
     * contrôle médian comme descripteur comportemental (~ milieu de la courbe).
     */
    private int computeGridCell (double [] x)
    {
        ...
        return gx * GRID_SIZE + gy;
    }

    /** Met à jour l'archive si l'individu est meilleur que l'occupant de sa case. */
    private void updateArchive (double [] x, double f)
    {
        int cell = computeGridCell (x);
        archiveVisits [cell]++;
        if (f < archiveFitness [cell])
        {
            archiveFitness [cell] = f;
            archiveSolutions [cell] = x.clone ();
        }
    }

    /**
     * Sélectionne une seed depuis l'archive QD :
     * - Parmi les cases occupées, on préfère celles avec peu de visites (exploration).
     * - On utilise un tirage proportionnel inversé aux nombre de visites.
     */
    private double [] selectFromArchive ()
    {
        // Collecter les cases occupées
        ...
        // Poids inversement proportionnels aux visites (1 / (1 + visits))
        double [] w = new double [occupied.size ()];
        double totalW = 0;
        for (int i = 0; i < occupied.size (); i++)
        {
            w [i] = 1.0 / (1.0 + archiveVisits [occupied.get (i)]);
            totalW += w [i];
        }
        ...
        for (int i = 0; i < occupied.size (); i++)
        {
            cumul += w [i];
            if (r <= cumul)
                return archiveSolutions [occupied.get (i)].clone ();
        }
        return archiveSolutions [occupied.get (occupied.size () - 1)].clone ();
    }
    ...
    private void setupCMAES (int newLambda, double [] startMean, double startSigma)
    {
        ...
        for (int i = 0; i < d; i++) { C [i][i] = 1.0; B [i][i] = 1.0; diagD [i] = 1.0; invsqrtC [i][i] = 1.0; }
    ...
    // ================================================================
    //  RESTART BIPOP + Quality-Diversity
    // ================================================================
    private void restart ()
    {
        ...
        if (restartCount % 2 == 1)
        {
            ...
            if (choice < 0.45)
            {
                // QD archive : seed peu explorée
                double [] archiveSeed = selectFromArchive ();
                if (archiveSeed != null)
                {
                    newMean = archiveSeed;
                    for (int i = 0; i < d; i++)
                        newMean [i] += rng.nextGaussian () * bounds.getRange (i) * 0.10;
                    bounds.clampInPlace (newMean);
                }
                else
                {
                    newMean = problem.getRandomControlPoints1DArray ();
                    bounds.clampInPlace (newMean);
                }
            }
        ...
        {
            // --- Small restart (exploitation locale) ---
            newLambda = lambda0;
    ...
    private void tred2 (double [][] V, double [] dArr, double [] e)
    {
        ...
        for (int i = n-1; i > 0; i--)
        {
            ...
            else
            {
                ...
                double f = dArr [i-1]; double g = Math.sqrt (h);
                if (f > 0) g = -g;
                e [i] = scale * g; h -= f * g; dArr [i-1] = f - g;
    ...
    private void tql2 (double [][] V, double [] dArr, double [] e)
    {
        ...
        for (int l = 0; l < n; l++)
        {
            ...
            if (m > l)
            {
                ...
                do
                {
                    ...
                    dArr [l] = e [l] / (p + r); dArr [l+1] = e [l] * (p + r);
                    double dl1 = dArr [l+1]; double h = g - dArr [l];
                    ...
                    for (int i = m-1; i >= l; i--)
                    {
                        ...
                        {
                            h = V [kk][i+1]; V [kk][i+1] = s * V [kk][i] + c * h;
                            V [kk][i] = c * V [kk][i] - s * h;
                        }
                    }
                    p = -s * s2 * c3 * el1 * e [l] / dl1; e [l] = s * p; dArr [l] = c * p;
                }
        ...
        for (int i = 0; i < n-1; i++)
        {
            ...
            if (k != i) { dArr [k] = dArr [i]; dArr [i] = p; for (int j = 0; j < n; j++) { p = V [j][i]; V [j][i] = V [j][k]; V [j][k] = p; } }
        }
    }
    ...
    private double [] matVecMul (double [][] M, double [] v)
    {
        ...
        for (int i = 0; i < d; i++) for (int j = 0; j < d; j++) r [i] += M [i][j] * v [j];
        return r;
    }

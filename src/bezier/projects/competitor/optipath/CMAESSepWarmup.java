...
/**
 * Sep-CMA-ES → Full CMA-ES avec transition adaptative et BIPOP restarts.
 *
 ...
 *   Durée : 300 générations ou jusqu'à stagnation de sep-phase.
 *
 * Phase 2 (Full CMA-ES) : C est initialisé avec la diagonale apprise + I.
 *   Permet d'apprendre les corrélations entre dimensions.
 *
 * Particulièrement efficace sur prob1 (d=32) où apprendre les 528 coefficients
 * de covariance dans le budget est coûteux.
 */
public class CMAESSepWarmup implements Optimizer
{
    ...
    // Sep-CMA-ES mode (diagonal only)
    private double [] diagC;     // diagonal of C in sep mode
    private double [] ps, pc;
    ...
    // Sep/Full mode control
    private boolean sepMode;
    private int sepGenerations;              // how many gens in sep-mode
    private static final int SEP_BUDGET = 300; // switch after this many sep gens
    private double [] learnedDiag;            // saved diagonal for transition

    public CMAESSepWarmup (Problem problem, int d, double [] lb, double [] ub, double [] initMean)
    {
    ...
    public void init ()
    {
        ...
        setupCMAES (lambda0, localBestX, sigma0, true); // Start in Sep mode
    }

    /**
     * Configure CMA-ES. If startInSep=true, starts in Sep-CMA-ES mode.
     */
    private void setupCMAES (int newLambda, double [] startMean, double startSigma, boolean startInSep)
    {
        ...
        if (sepMode)
        {
            // Sep mode: only diagonal
            diagC = new double [d];
            Arrays.fill (diagC, 1.0);
            diagD = new double [d];
            Arrays.fill (diagD, 1.0);
            // No B, C, invsqrtC needed in sep mode
            B = null; C = null; invsqrtC = null;
        }
        else
        {
            diagC = null;
    ...
    {
        // Save learned diagonal
        learnedDiag = diagC.clone ();

        // Create full C = 0.3 * diag(diagC) + 0.7 * I
        C = new double [d][d]; B = new double [d][d];
        invsqrtC = new double [d][d];
        for (int i = 0; i < d; i++)
        {
            C [i][i] = 0.3 * learnedDiag [i] + 0.7;
            B [i][i] = 1.0;
        ...
        // Reset evolution paths (partial reset to preserve some momentum)
    ...
    /** Sep-CMA-ES generation: O(d) covariance update. */
    private void stepSep ()
    {
        double [][] arx = new double [lambda][d];
        double [][] arz = new double [lambda][d]; // z_k = raw gaussians
        ...
        for (int k = 0; k < lambda; k++)
        {
            ...
            // x = mean + sigma * diag(sqrt(diagC)) * z
            for (int i = 0; i < d; i++)
                arx [k][i] = mean [i] + sigma * diagD [i] * z [i];
        ...
        // Update ps (CSA) — in sep mode, invsqrtC is just diag(1/diagD)
        ...
        // Sep covariance update: only diagonal
        ...
        // Check for transition to full CMA-ES
    ...
    /** Full CMA-ES generation: O(d^2) covariance update. */
    ...
    private boolean shouldRestart ()
    {
        ...
        if (sepMode)
        {
            // In sep mode, check if diagonal is degenerate
    ...
    private void restart ()
    {
        ...
        // Always restart in Sep mode (re-learn diagonal quickly)
        setupCMAES (newLambda, newMean, newSigma, true);
    }

    // ================================================================
    //  Seeds, local opt — standard
    // ================================================================
    private ArrayList<double []> generateDiverseSeeds ()
    {
        ...
        {double[]path=new double[d];for(int i=0;i<nCP;i++){double t=(double)(i+1)/(nCP+1);path[2*i]=startX+t*(endX-startX);path[2*i+1]=centerY+amp*halfY*Math.sin(2*Math.PI*period*t+phase);}seeds.add(path);}

        for(int nUp=1;nUp<=nCP/2;nUp++){double[]pathA=new double[d],pathB=new double[d];for(int i=0;i<nCP;i++){double t=(double)(i+1)/(nCP+1);boolean up=((i/nUp)%2==0);pathA[2*i]=startX+t*(endX-startX);pathA[2*i+1]=up?(ubY-1):(lbY+1);pathB[2*i]=startX+t*(endX-startX);pathB[2*i+1]=up?(lbY+1):(ubY-1);}seeds.add(pathA);seeds.add(pathB);}

        for(double yy:new double[]{lbY+1.5,ubY-1.5,centerY}){double[]path=new double[d];for(int i=0;i<nCP;i++){double t=(double)(i+1)/(nCP+1);path[2*i]=startX+t*(endX-startX);path[2*i+1]=yy;}seeds.add(path);}
        ...
        for(double yVal:yLevels){double[]path=new double[d];for(int i=0;i<nCP;i++){path[2*i]=(i%2==0)?ubX:lbX;path[2*i+1]=yVal;}seeds.add(path);}

        for(double y1:new double[]{lbY,lbY+1,lbY+3})for(double y2:new double[]{lbY,lbY+2,lbY+5,centerY}){double[]path=new double[d];for(int i=0;i<nCP;i++){path[2*i]=(i%2==0)?ubX:lbX;path[2*i+1]=(i<nCP/2)?y1:y2;}seeds.add(path);}

        for(double xVal:new double[]{lbX,ubX})for(double yVal:yLevels){double[]path=new double[d];for(int i=0;i<nCP;i++){path[2*i]=xVal;path[2*i+1]=yVal;}seeds.add(path);}

        for(int r=0;r<30;r++){double[]path=new double[d];for(int i=0;i<nCP;i++){path[2*i]=rng.nextBoolean()?lbX:ubX;path[2*i+1]=lbY+rng.nextDouble()*(ubY-lbY);}seeds.add(path);}
    ...
    private double [] quickLocalOptimize (double [] x0, int maxEvals)
    {
        ...
        for(int e=0;e<maxEvals;e++){double[]xNew=new double[d];for(int i=0;i<d;i++)xNew[i]=x[i]+rng.nextGaussian()*step;bounds.clampInPlace(xNew);double fNew=problem.evaluate(xNew);updateBest(xNew,fNew);if(fNew<fx){x=xNew;fx=fNew;successes++;}window++;if(window>=20){double rate=(double)successes/window;if(rate>0.2)step*=1.3;else step*=0.7;step=Math.max(step,1e-10);step=Math.min(step,sigma0*2);successes=0;window=0;}}
        return x;
    }

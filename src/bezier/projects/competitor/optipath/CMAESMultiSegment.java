...
/**
 * CMA-ES avec representation en spline multi-segments de Bezier.
 *
 * Au lieu d'optimiser directement les points de controle d'une Bezier unique,
 * le vecteur de decision encode une spline a 2 segments avec continuite C1
 * a la jonction. Le vecteur est converti en CPs standard avant chaque evaluation
 * via MultiSegmentBezier.toControlPoints().
 *
 * Le reste de l'algorithme (echantillonnage, mise a jour, eigendecomposition,
 * restarts IPOP) est identique a CMAESOptimizer.
 */
public class CMAESMultiSegment implements Optimizer
{
    // ===== References externes =====
    ...
    // ===== Multi-segment =====
    private final MultiSegmentBezier multiSeg;
    ...
    // ===== Constructeur =====
    public CMAESMultiSegment (Problem problem, int d, double [] lb, double [] ub, double [] initMean)
    {
        ...
        // Initialiser le convertisseur multi-segment
        this.multiSeg = new MultiSegmentBezier (d / 2,
                problem.getStartPoint ().getX (), problem.getStartPoint ().getY (),
                problem.getEndPoint ().getX (),   problem.getEndPoint ().getY ());

        // Convertir l'initMean standard en format multi-segment
        this.initMean = multiSeg.fromStandardCPs (initMean);
    }

    // ================================================================
    //  EVALUATION (passe par la conversion multi-segment)
    // ================================================================
    private double evaluateMultiSeg (double [] msVec)
    {
        double [] standardCPs = multiSeg.toControlPoints (msVec);
        bounds.clampInPlace (standardCPs);
        return problem.evaluate (standardCPs);
    }
    ...
    public void init ()
    {
        ...
        // Generer des seeds standard puis convertir en multi-segment
        ArrayList<double []> standardSeeds = generateDiverseSeeds ();

        // Ajouter l'interpolation lineaire en standard
        int nCP = d / 2;
        double [] stdLinear = new double [d];
        for (int i = 0; i < nCP; i++)
        {
            double t = (double) (i + 1) / (nCP + 1);
            stdLinear [2 * i]     = startX + t * (endX - startX);
            stdLinear [2 * i + 1] = startY + t * (endY - startY);
        }
        standardSeeds.add (0, stdLinear);
        ...
        for (double [] stdSeed : standardSeeds)
        {
            double [] msSeed = multiSeg.fromStandardCPs (stdSeed);
            bounds.clampInPlace (msSeed);
            double f = evaluateMultiSeg (msSeed);
            updateBest (msSeed, f);
            evaluated.add (msSeed);
            fitnesses.add (f);
            if (f < bestSeedF)
            {
                bestSeedF = f;
                bestSeed = msSeed.clone ();
            }
        }
        ...
        // Optimisation locale rapide sur les top-5 seeds
        ...
        for (int s = 0; s < nLocalSeeds; s++)
        {
            ...
            double fResult = evaluateMultiSeg (result);
    ...
    // ================================================================
    //  GENERATION DE SEEDS (standard CPs, convertis ensuite)
    // ================================================================
    ...
    private double [] quickLocalOptimize (double [] x0, int maxEvals)
    {
        ...
        for (int e = 0; e < maxEvals; e++)
        {
            ...
            double fNew = evaluateMultiSeg (xNew);
    ...
    // ================================================================
    //  UNE GENERATION (evaluation via multi-segment)
    // ================================================================
    @Override
    public void step ()
    {
        ...
        for (int k = 0; k < lambda; k++)
        {
            ...
            fitness [k] = evaluateMultiSeg (arx [k]);
    ...
    // ================================================================
    //  STAGNATION + RESTART IPOP (seeds converties en multi-segment)
    // ================================================================
    ...
    private void restart ()
    {
        ...
        {
            // Seeds standard converties en multi-segment
            double [] stdBSeed = problem.getRandomControlPoints1DArray ();
            double [] bSeed = multiSeg.fromStandardCPs (stdBSeed);
            bounds.clampInPlace (bSeed);
            double bF = evaluateMultiSeg (bSeed);
            updateBest (bSeed, bF);
            for (int r = 0; r < 3; r++)
            {
                double [] stdS = problem.getRandomControlPoints1DArray ();
                double [] s = multiSeg.fromStandardCPs (stdS);
                bounds.clampInPlace (s);
                double f = evaluateMultiSeg (s);
                updateBest (s, f);
            ...
            for (int r = 0; r < 4; r++)
            {
                ...
                for (int i = 0; i < nCP; i++)
                {
                    stdS [2 * i]     = rng.nextBoolean () ? bounds.getLb () [0] : bounds.getUb () [0];
                    stdS [2 * i + 1] = bounds.getLb () [1] + rng.nextDouble () * bounds.getRange (1);
                }
                double [] s = multiSeg.fromStandardCPs (stdS);
                bounds.clampInPlace (s);
                double f = evaluateMultiSeg (s);
    ...
    // ================================================================
    //  EIGENDECOMPOSITION (identique — tred2 + tql2, JAMA domaine public)
    // ================================================================

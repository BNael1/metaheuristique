...
/**
 * CMAESFinal — Fusion of the best components from all variants.
 *
 * Combines:
 *   1. BIPOP dual-regime restarts (large exploration + small exploitation)
 *   2. Active CMA-ES with negative weights (Jastrebski & Arnold 2006)
 *   3. Covariance Memory across restarts (C = alpha*C_prev + (1-alpha)*I)
 *   4. Mirror Sampling (Brockhoff et al. 2010) — halves gradient variance
 *   5. Surrogate RBF Pre-Screening with adaptive quality tracking (Kendall tau)
 *      + safety fraction to hedge against bad surrogate predictions
 *
 * The surrogate is self-regulating: if its rank correlation with true fitness
 * drops below a threshold, it auto-disables and falls back to standard CMA-ES.
 * This prevents performance degradation on obstacle-heavy landscapes.
 */
public class CMAESFinal implements Optimizer
{
    // ===== External references =====
    ...
    private int lambda;
    private int mu;
    private double [] weightsPositive;
    private double [] weightsNegative;
    ...
    // ===== Constructor =====
    public CMAESFinal (Problem problem, int d, double [] lb, double [] ub, double [] initMean)
    {
    ...
    // ================================================================
    //  SETUP CMA-ES with Active Weights + Covariance Memory
    // ================================================================
    ...
    // ================================================================
    //  ONE GENERATION — Mirror Sampling + Active Update + Surrogate
    // ================================================================
    @Override
    public void step ()
    {
        ...
        if (useSurrogate)
        {
            // --- SURROGATE + MIRROR SAMPLING GENERATION ---
            int nSafe = Math.max (2, (int) (SAFETY_FRACTION * lambda));
            int nScreen = lambda - nSafe;
            // Generate PRESCREENING_FACTOR * lambda candidates using mirror sampling
            int nCandidates = PRESCREENING_FACTOR * lambda;
            // Ensure even for mirroring
            if (nCandidates % 2 != 0) nCandidates++;
            int halfCand = nCandidates / 2;

            double [][] candX = new double [nCandidates][d];
            double [][] candY = new double [nCandidates][d];

            for (int k = 0; k < halfCand; k++)
            {
                ...
                {
                    candY [km][i] = -y [i];
                    candX [km][i] = mean [i] - sigma * y [i];
                }
                bounds.clampInPlace (candX [km]);
            }
            ...
            // Safety fraction from unselected pool
        ...
        {
            // --- STANDARD MIRROR SAMPLING (no surrogate) ---
            ...
            for (int k = 0; k < halfLambda; k++)
            {
                ...
                surrogate.addPoint (arx [km], fitness [km]);
            }
        }
        ...
        // New mean (positive weights only)
        ...
        // ACTIVE covariance matrix update
        ...
        for (int i = 0; i < d; i++)
        {
            for (int j = 0; j <= i; j++)
            {
                ...
                // ACTIVE negative (worst mu) with Mahalanobis normalization
                double rankmuNeg = 0;
        ...
        // Stagnation detection + BIPOP restart
    ...
    // ================================================================
    //  BIPOP RESTART with Covariance Memory
    // ================================================================
    private void restart ()
    {
        restartCount++;

        // Save covariance matrix for memory
        savedC = new double [d][d];
        for (int i = 0; i < d; i++)
            for (int j = 0; j < d; j++)
                savedC [i][j] = C [i][j];

        // Clear surrogate (stale data after restart)
        surrogate.clear ();
        surrogateQuality = 0.5;

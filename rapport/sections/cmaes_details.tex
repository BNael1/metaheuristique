...
diversifié de \textbf{graines candidates} (\textit{seeds}) puis sélectionnons la meilleure
...
\subsection{Stratégies de Restart (IPOP et BIPOP)}
\label{sec:ipop-bipop}

Lorsque la convergence est détectée, l'algorithme effectue un \textit{restart}.
Trois critères d'arrêt sont vérifiés à chaque génération :
\begin{enumerate}[nosep]
  \item \textbf{Stagnation de la \textit{fitness}} : l'écart entre le meilleur
        et le pire individu est inférieur à $10^{-12}$, seuil choisi proche de
        la précision machine double ($\approx 2{,}2 \times 10^{-16}$) ;
  \item \textbf{Effondrement de $\sigma$} : $\sigma < 10^{-16}$, indiquant
        que le pas de recherche est trop petit pour progresser ;
  \item \textbf{Dégénérescence de $\mathbf{C}$} : le rapport
        $\lambda_{\max}/\lambda_{\min}$ des valeurs propres de $\mathbf{C}$
        dépasse $10^{14}$, signalant un conditionnement numérique pathologique.
\end{enumerate}
Deux stratégies de \textit{restart} ont été implémentées :

\paragraph{IPOP (Increasing Population Size).}
La taille de population est doublée à chaque redémarrage ($\lambda_{new} = 2 \lambda_{old}$), plafonnée à 512. Cela permet d'explorer des bassins d'attraction de plus en plus larges.

\paragraph{BIPOP (Bi-Population)~\cite{hansen2009}.}
Cette variante alterne entre deux régimes pour maximiser l'efficacité du budget d'évaluations restant :
\begin{itemize}[nosep]
    \item \textbf{Exploration (Large)} : Comme IPOP, on double la population pour sortir des bassins d'attraction locaux.
    \item \textbf{Exploitation (Small)} : On utilise une petite population ($\lambda_0$) avec un pas de recherche réduit ($\sigma = \sigma_0 / 100$) pour converger rapidement vers des optima locaux potentiellement manqués.
\end{itemize}
...
\begin{enumerate}[nosep]
  ...
  \item[30\%] meilleur parmi 8~candidats (4~aléatoires + 4~regroupements aux frontières).
\end{enumerate}
